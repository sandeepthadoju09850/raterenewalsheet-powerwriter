...
public class CU09_KS_CURegression extends BaseTest{
	...
	public static void CU09_KS_CURegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,String strRegressionName, File file,XSSFWorkbook workbook, ExtentTest test) throws IOException 
	{
		JavascriptExecutor executor = (JavascriptExecutor) se.driver();
		OR_CP ORCP = TestPageFactory.initElements(se, OR_CP.class);CommonAPMethods BT=TestPageFactory.initElements(se, CommonAPMethods.class);
		...
		CP_AccountClearance CP_AccountClearance = TestPageFactory.initElements(se, CP_AccountClearance.class);CP_BuildingandOccupantSelection CP_BandOS = TestPageFactory.initElements(se, CP_BuildingandOccupantSelection.class);
		...
		try {
			...
			String strRegressionIDUnderlined = "CUTC_14";
			...
			// UNDERLYING POLICY 1 CUTC_09 NEW QUOTE TRANSACTION
			...
			strRegressionIDUnderlined = "CUCA_08";
			...
			//	CP_ExtendedNamedInsured.CP_ExtendedNamedInsured_Details(strRegressionIDUnderlined, transaction, suspendSheet, strAgentLink,strLOB,strRelease_SelectRelease,strRole_SelectRoleAs, test);
				...
				CA_BusinessInterruption.CA_BusinessInterruptionDetails(strRegressionIDUnderlined, transaction, suspendSheet, test);
				//CA_Garagekeepers.CA_Garagekeepers_Details(strRegressionIDUnderlined, transaction, suspendSheet, test); work around as part of - DF AP-43631
			...
			if (transactionsList.contains("NewQuote")) {
				...
				BT.ClickContinue(test);//to cntinue on Prem Mod screen
			...
			// Booking Underlying Quote (KY TC) in PW
			...
			if (transactionsList.contains("BookPolicyInPW")) {
				...
				strRegressionIDUnderlined = "CUTC_14";
				LoginPage.APAppLogin(strRegressionIDUnderlined,   transaction,"No", test);
			...
			// Policy Issuance of Underlying Quote (KS TC)
			...
			// Policy Issuance of Underlying Quote (KS CA)
			if (transactionsList.contains("PolicyIssuanceOnAP")) {
			...
			// Policy Issuance of CU Quote (KS CU) 
			if (transactionsList.contains("PolicyIssuanceOnAP")) {
			...
			e.printStackTrace();
			se.verify().verifyEquals("CU03_KY_CURegression test  failed", true, false, true, test);
