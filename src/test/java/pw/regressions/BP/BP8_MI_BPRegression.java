...
import pw.pages.CP.CP_Spoilage;
...
public class BP8_MI_BPRegression extends BaseTest {
	ExtentReports extent;
	ExtentTest test;
	
	
	public static void MIBPRegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,File file,XSSFWorkbook workbook, ExtentTest test) throws IOException {
		...
		try {
			...
			if(transactionsList.contains("ConvertQFRToPolicy")){
				...
				CommonMethods.NavigateToPageContaingText("Loc 1 Bldg 1 - Carson City, MI", test);
				CommonMethods.NavigateTo("Spoilage (1)", test);
				CPSpoilage.Spoilage(strRegressionID, transaction, "Location 1 Building 1", test);
				CommonMethods.getCalculatePremium(strRegressionID,transaction,test);	
				PremiumSummary.GetPremiumSummary(strRegressionID,transaction,transactionsList,file, workbook,test);
				CommonMethods.getDashboardActions(strRegressionID,transaction,test);
			...
			e.printStackTrace();
			se.verify().verifyEquals("MI BP Regression test  failed", true, false);
			testTearDown(se);
		}
	}
