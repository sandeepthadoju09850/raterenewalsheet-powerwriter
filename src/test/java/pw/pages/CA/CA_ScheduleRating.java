...
public class CA_ScheduleRating extends OR_CA  {
	
	public void CA_ScheduleRatingPage(String strRegressionID,String transaction,String suspendSheet,ExtentTest test) throws IOException{
		try{
			if(suspendSheet.equalsIgnoreCase(constants.ScheduleRating)){
		    	test.log(LogStatus.INFO, "Page Suspended :- ","Transaction Step : "+ transaction + "<br>" +"Page : "+suspendSheet);
		         testTearDown(se, test);
		    }
		    
			List<Map<String, String>> table = ExcelOperations.getPagesData(constants.ScheduleRating, strRegressionID, transaction);
			int iteration = 0;
		...
		try{
			...
			*/String strScheduleRating_TXT_ScheduleRisk_SelectedModifier = (String) row.get("ScheduleRating_TXT_ScheduleRisk_SelectedModifier");
			...
			se.log().logTestStep("CA_ScheduleRating");
			test.log(LogStatus.INFO, "CA_ScheduleRating page","Transaction Step : "+transaction + "<br>" +"Page : CA_ScheduleRating");
			se.verify().verifyEquals("CA_ScheduleRating of PowerWriter ", getScheduleRating_LBL_ScheduleRating().isDisplayed(),true, true,test);
			...
			se.element().enterOrValidateText(getScheduleRating_TXT_ScheduleRisk_SelectedModifier(strScheduleRating_TXT_ScheduleRisk_Coverage,strScheduleRating_TXT_RiskCharacteristic,strScheduleRating_TXT_ScheduleRisk_SelectedModifier),strScheduleRating_TXT_ScheduleRisk_SelectedModifier,test);
			se.element().enterOrValidateText(getScheduleRating_TXT_ScheduleRisk_Reason(strScheduleRating_TXT_ScheduleRisk_Coverage,strScheduleRating_TXT_RiskCharacteristic,strScheduleRating_TXT_ScheduleRisk_Reason),strScheduleRating_TXT_ScheduleRisk_Reason,test);
			...
			se.verify().verifyEquals("Failed to fill in details for CAScheduleRating", true, false, true, test);
