...
public class CP2_CO_CPRegression extends BaseTest{

	ExtentReports extent;
	ExtentTest test;
	public static void COCPRegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params, String strRegressionID,File file,XSSFWorkbook workbook,ExtentTest test) throws IOException {
		...
		CP_FungusWet FungusWet  =  TestPageFactory.initElements(se, CP_FungusWet.class);
		...
		CIM_ContractorsEquipment CIM_ContractorsEquipmentPage = TestPageFactory.initElements(se, CIM_ContractorsEquipment.class);
		CIM_ContractorsOtherCoverages CIM_ContractorsOtherCoveragesPage = TestPageFactory.initElements(se, CIM_ContractorsOtherCoverages.class);
		...
		try {
			...
			if(transactionsList.contains("NewQuote")){
				...
				FungusWet.FungusWet(strRegressionID, transaction, test);
				...
				CIM_ContractorsEquipmentPage.ContractorsEquipment(strRegressionID, transaction, test);
				CIM_ContractorsOtherCoveragesPage.CIMContractorsOtherCoverages(strRegressionID, transaction, test);
				CommonMethods.NavigateToNextScreen(test);
			...
			if(transactionsList.contains("FlatEndorsement1")){
				...
				CommonMethods.NavigateTo("Fungus, Wet Rot, Dry Rot and Bacteria Coverage",test);
				FungusWet.FungusWet(strRegressionID, transaction, test);
			...
			if(transactionsList.contains("QuoteForRenewal")){
				...
				CommonMethods.NavigateToPageContaingText("Business Income (1)",test);
				BusinessIncome.BusinessIncome(strRegressionID, transaction,"Location 1 Building 1", test);
				CommonMethods.CollapseAllAndNavigateTo("Commercial Package","20-CP",test);
				CommonMethods.NavigateTo("Commercial Property",test);
				CommonMethods.NavigateTo("Buildings (2)",test);
				CommonMethods.NavigateToPageContaingText("Loc 1 Bldg 2",test);
				CommonMethods.NavigateToPageContaingText("Business Income (1)",test);
				BusinessIncome.BusinessIncome(strRegressionID, transaction,"Location 1 Building 2", test);
				CommonMethods.CollapseAllAndNavigateTo("Commercial Package","20-CP",test);
				CommonMethods.NavigateTo("General Liability",test);
			...
			e.printStackTrace();
			se.verify().verifyEquals("CO_CPRegressionMethod test  failed", true, false);
			testTearDown(se);
		}
	}
}
