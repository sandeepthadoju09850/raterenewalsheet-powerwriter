...
public class CommonBaseTest {
	...
	protected static void testTearDown(SeHelper se) {
		se.verify().checkForFail();
	}
	...
	public static Map<String, Object> getXLSTestData(String filename, String sheetVersion, String sheetName){
		...
		if(sheetVersion.equalsIgnoreCase("new")){
			...
			String scenario = "_"+System.getProperty("userName")+".xlsx";
			...
			try{
		        ...
		        testTable = xlsdata.parseXLSXSheet(filenam, sheetName);
			}catch(Exception e){				 
				if(filename.contains("Scenario_Selection")){
	...
	public Object[][] XLSDataProviderByRow(Method method) {
		...
		Object testObject = getXLSTestDataByRow(method.getName().substring(0, 2),testData.fileName(), testData.sheetVersion(), testData.sheetName()).get("test");
