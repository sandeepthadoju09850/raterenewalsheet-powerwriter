...
import pw.pages.CA.CA_DriveAwayContractorsLiability;
import pw.pages.CA.CA_ExperienceRating;
import pw.pages.CA.CA_FinancialResponsibilityFiling;
...
public class CA30_WIBusinessAuto_CARegression  extends BaseTest{

	ExtentReports extent;
	ExtentTest test;
		
	public static void WIBusinessAuto_CARegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,File file,XSSFWorkbook workbook, ExtentTest test) throws IOException {
		...
		CA_DriveAwayContractorsLiability CA_DriveAwayContractorsLiabilityPage = TestPageFactory.initElements(se, CA_DriveAwayContractorsLiability.class);
		CA_FinancialResponsibilityFiling CA_FinancialResponsibilityFilingPage = TestPageFactory.initElements(se, CA_FinancialResponsibilityFiling.class);
		...
		try {
			...
			if (transactionsList.contains("NewQuote")) {
				...
				CA_DriveAwayContractorsLiabilityPage.DriveAwayContractorsLiability(strRegressionID, transaction, suspendSheet, test);
				CA_FinancialResponsibilityFilingPage.FinancialResponsibilityFiling(strRegressionID, transaction, suspendSheet, test);
			...
			e.printStackTrace();
			se.verify().verifyEquals("WIBusinessAutoRegression test  failed", true, false);
			testTearDown(se);
		}
	}
