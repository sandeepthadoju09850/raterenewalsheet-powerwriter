...
public class CAWC_PW_Producers extends OR_Common {
	...
	public void AddProducers(String strRegressionID, String transaction, ExtentTest test) throws IOException {
		...
		String strProducers_TXT_StateUnit = (String) row.get("Producers_TXT_SteUnit");
		...
		String strProducers_BTN_Delete = (String) row.get("Producers_BTN_Delete");
		String strProducers_BTN_EditProducer  = (String) row.get("Producers_BTN_EditProducer");
		String strProducers_CHK_RowNo_Select = (String) row.get("Producers_CHK_RowNo_Select");
		...
		try{
			...
			if(strProducers_BTN_Delete.equalsIgnoreCase("Yes")){
				se.element().checkUncheckOrValidate(getProducers_CHK_Row_Select(strProducers_CHK_RowNo_Select),"Check",test);
				se.element().Click(getProducers_BTN_Delete(strProducers_BTN_Delete), test);
			}	
		...
		//	se.element().waitForElementIsDisplayed(getProducers_Producer_LNK_popup(strProducers_TXT_Producer.toUpperCase());
			...
			se.element().enterOrValidateText(getProducers_TXT_SteUnit(strProducers_TXT_StateUnit),strProducers_TXT_StateUnit,test);
			...
			se.verify().verifyEquals("Failed to fill in details for Producers ", true, false, true, test);
