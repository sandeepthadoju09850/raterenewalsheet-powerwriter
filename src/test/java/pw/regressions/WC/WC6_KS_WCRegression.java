...
public class WC6_KS_WCRegression extends BaseTest {
	ExtentReports extent;
	ExtentTest test;
	public static void KS_WCRegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,File file,XSSFWorkbook workbook, ExtentTest test) throws IOException {
		...
		try {
			...
			if(transactionsList.contains("ProRataEndorsement1")){
				...
				WC_LocationsPage.Locations(strRegressionID,"ProRataEndorsement1a",suspendSheet,test);
			...
			e.printStackTrace();
			se.verify().verifyEquals("KS WC Regression test  failed", true, false);
			testTearDown(se);
		}
	}
