...
import Libraries.automation.common.framework.*;
import Libraries.automation.common.framework.Browser;
...
public class SeHelper {
     ...
     */
    public Browsers currentBrowser() { return currentBrowser; }
    /**
     ...
     */
    public Verification verify() { return verification; }
    /**
...
//				chromeOptions1.addArguments("--proxy-bypass-list=*");
			...
			case VMIE:
				...
				break;	
			/*
			 * case GridChrome: //capabilities = DesiredCapabilities.chrome();
			 * capabilities.setBrowserName("chrome");
			 * capabilities.setPlatform(Platform.WINDOWS); ChromeOptions options = new
			 * ChromeOptions();
			 * options.setBinary("C:/NewGC/Google1/Chrome/Application/chrome.exe");
			 * options.merge(capabilities); driver = new RemoteWebDriver(getSeGridUrl(),
			 * options); //System.setProperty("Webdriver.chorme.driver",
			 ...
			 * ; //addChromeExtension(capabilities); //driver = (new
			 * Augmenter()).augment(new RemoteWebDriver(getSeGridUrl(), capabilities));
			...
			/*
			 * case GridInternetExplorer: //capabilities =
			 ...
			 * capabilities.setCapability("ignoreZoomSetting", true);
			 * capabilities.setCapability("ie.ensureCleanSession", true); //
			 * IEOptions.merge(capabilities); driver = (new Augmenter()).augment(new
			 * RemoteWebDriver( getSeGridUrl(), capabilities));
			 * ((RemoteWebDriver)driver).setFileDetector(new LocalFileDetector()); break;
			 */
			 ...
			 * capabilities.setCapability("version", "44.0"); se.util().sleep(15000);
			 * seMethodCount++; se.log.logSeStep(" array values " + seMethodCount + " - " +
			 * CommonBaseTest.myMethods[6]); String sMethodName =
			 * CommonBaseTest.myMethods[seMethodCount];
			 * 
			 * capabilities.setCapability("name", sMethodName + " by " +
			 * sUserName+" on "+SEnvname); chromeOptions.merge(capabilities);
			 * 
			 * try { driver = new RemoteWebDriver(new URL(URL), capabilities);
			 * ((RemoteWebDriver)driver).setFileDetector(new LocalFileDetector()); } catch
			 * (MalformedURLException e) { // TODO Auto-generated catch block
			 * e.printStackTrace(); } break;
			 */

			/*
			 * case SauceIE: //capabilities = DesiredCapabilities.internetExplorer();
			 ...
			 * se.util().sleep(15000);
			 * 
			 * seMethodCount++; String sMethodNameIE =
			 * CommonBaseTest.myMethods[seMethodCount];
			 * se.log.logSeStep(" Method name in WebDriver : " + sMethodNameIE);
			 * 
			 * capabilities.setCapability("name", sMethodNameIE + " by " +
			 * sUserName+" on "+SEnvname); //IEOptions.merge(capabilities); try { driver =
	...
	/*
	 * private URL getSeGridUrl() { URL seGrid = null; try { seGrid = new
	 * URL(settings().value("sehelper.seleniumGrid")); } catch
	 * (MalformedURLException e) { e.printStackTrace(); } return seGrid; }
	 */
