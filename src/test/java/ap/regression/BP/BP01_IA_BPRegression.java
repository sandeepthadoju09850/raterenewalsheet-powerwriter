...
public class BP01_IA_BPRegression extends BaseTest{
	...
	public static void BP01_IA_BPRegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,String strRegressionName, File file,XSSFWorkbook workbook, ExtentTest test) throws IOException {
		{
			...
			try {
				...
				while (iteration < table.size()) {
					...
					if (transactionsList.contains("NewQuote")) {
						...
						CP_AccReceivableSchedule.CP_AccReceivableSchedule_Details(strRegressionID, transaction, suspendSheet, test);
						
						CP_ContractorsEquipmentScheduled.CP_ContractorsEquipmentScheduled_Details(strRegressionID, transaction, suspendSheet, test);
						CP_SignsScheduleInfo.CP_SignsScheduleInfo_Details(strRegressionID, transaction, suspendSheet, test);
						CP_ValuablePapers.CP_ValuablePapers_Details(strRegressionID, transaction, suspendSheet, test);
					...
					if (transactionsList.contains("RedoNewQuoteOne")) {
						transaction = "RedoNewQuoteOne";
						String suspendSheet = ExcelOperations.getPageToBeSuspended(strRegressionID,transaction);
						APPW_CommonMethods.refreshWhen502error("BP_01 RedoNewQuoteOne summary ", 2,  test);
						...
						BT.ClickContinue(test);
						//BT.ClickContinue(test);
						//BT.NavigateToTabs("Billing, Supporting Policies and Contacts",test);
						CP_BillingInformation.CP_BillingInformation_Details(strRegressionID, transaction, suspendSheet, test);
				...
				e.printStackTrace();
				se.verify().verifyEquals("BP01_IA_BPRegression test  failed", true, false, true, test);
