...
public class Functional_11_SL_PropertyCovDefaults_RoofSurfacingActualCashValue_Checked extends BaseTest{
	...
	public static void SL_PropertyCovDefaults_RoofSurfacingActualCashValue_Checked(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,String strRegressionName, File file,XSSFWorkbook workbook, ExtentTest test) throws IOException {
		{
			...
			try {
				...
				while (iteration < table.size()) {
					...
					if (transactionsList.contains("NewQuote")) {
						...
						CP_PropertyCoverages.CP_PropertyCoverages_Details(strRegressionID, transaction, suspendSheet, test); //Navigates to Optional Property Coverages page
						BT.ClickContinue(test); // Navigating from Optional Property Coverages page
						BT.ClickContinue(test); // Navigating from Liability Coverages page
						CP_Liablityclasses.CP_Liablityclasses_Details(strRegressionID, transaction, suspendSheet, test); //Navigates to Liability Additional Interests page
						BT.ClickContinue(test); // Navigating from Liability Options page
						BT.ClickContinue(test); // Navigating from Liability Additional Interests page
						BT.ClickContinue(test); //Navigating from Additional Coverage Parts page
						...
						BT.ClickContinue(test); //Navigating from Loss History page
						BT.ClickContinue(test); //Navigating from Premium Modification page
						...
						BT.ClickContinue(test); //Navigating from Property Additional Interests page
						BT.ClickContinue(test); //Navigating from Prior Carrier Information page
				...
				e.printStackTrace();
				se.verify().verifyEquals("Functional_11_SL_PropertyCovDefaults_RoofSurfacingActualCashValue_Checked test failed", true, false, true, test);
