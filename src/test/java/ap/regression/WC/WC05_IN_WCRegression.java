...
public class WC05_IN_WCRegression extends BaseTest {
	...
	public static void WC05_IN_WCRegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,String strRegressionName, File file,XSSFWorkbook workbook, ExtentTest test) throws IOException {
		{
			...
			try {
				...
				while (iteration < table.size()) {
					...
					if (transactionsList.contains("NewQuote")) {
						...
						CP_GenInfo.CP_GenInfo_Details(strRegressionID, "NewQuote1", suspendSheet, strAgentLink, strLOB, strRelease_SelectRelease, strRole_SelectRoleAs, test);
						BT.ClickContinue(test);
						BT.ClickContinue(test);
						WC_StateRatingFactors.WC_StateRatingFactors_Details(strRegressionID, transaction, suspendSheet, test);
						//WC_FlatWaiverOfSubrogation.WC_FlatWaiverOfSubrogation_Details(strRegressionID, transaction, suspendSheet, test);
				...
				e.printStackTrace();
				se.verify().verifyEquals("WC05_IN_WCRegression test  failed", true, false, true, test);
