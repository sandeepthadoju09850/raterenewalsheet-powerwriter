...
public class CU10_IL_CURegression extends BaseTest{
	...
	public static void CU10_IL_CURegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,String strRegressionName, File file,XSSFWorkbook workbook, ExtentTest test) throws IOException 
	{
		...
		try {
			...
			String strRegressionIDUnderlined = "CUBP_12";
			...
			// UNDERLYING POLICY 1 CUBP_12 NEW QUOTE TRANSACTION
			 ...
			 // UNDERLYING POLICY 1 CUBP_12 REDO TRANSACTION
			if (transactionsList.contains("RedoNewQuote")) {
				...
				CP_LiabilityAdditonalInterests.CP_LiabilityAdditonalInterests_Details(strRegressionIDUnderlined, transaction, suspendSheet, test);
				BT.ClickContinue(test); //Additional Coverage Parts
				BT.ClickContinue(test); //Crime Coverage Selection
				BT.ClickContinue(test); //Inland Marine Coverage Selection
				BT.ClickContinue(test); //Accounts Receivable Schedule
				BT.ClickContinue(test); //Contractors Equipment Schedule
				BT.ClickContinue(test); //Scheduled Property Floater
				BT.ClickContinue(test); //Signs Schedule
				BT.ClickContinue(test); //Valuable Papers Schedule
				CP_PremiumIndication.CP_PremiumIndication_Details(strRegressionIDUnderlined, transaction, suspendSheet, test);
			...
			// UNDERLYING POLICY 2 CUCA_09 NEW QUOTE TRANSACTION
			strRegressionIDUnderlined = "CUCA_09";
			...
			// UNDERLYING POLICY 3 CUWC_09 NEW QUOTE TRANSACTION
			strRegressionIDUnderlined = "CUWC_09";
			if (transactionsList.contains("NewQuote")) {
				transaction = "NewQuote";String suspendSheet = ExcelOperations.getPageToBeSuspended(strRegressionID,transaction);
				//LoginPage.APAppLogin(strRegressionIDUnderlined,   transaction,"No", test);
				APPW_CommonMethods.openQuoteInAP(quote,"N/A",test);
			...
			// COMMERCIAL UMBRELLA CU_10 NEW QUOTE TRANSACTION
			if (transactionsList.contains("NewQuote")) {
				transaction = "NewQuote";String suspendSheet = ExcelOperations.getPageToBeSuspended(strRegressionID,transaction);
				//LoginPage.APAppLogin(strRegressionIDUnderlined,   transaction,"No", test);
				APPW_CommonMethods.openQuoteInAP(quote,"N/A",test);
			...
			// Booking Underlying Quote (IL BP) in PW
			...
			if (transactionsList.contains("BookPolicyInPW")) {
				strRegressionIDUnderlined = "CUBP_12";
				LoginPage.APAppLogin(strRegressionIDUnderlined,   transaction,"No", test);
			...
			// Booking Underlying Quote (IL CA) in PW
			if (transactionsList.contains("BookPolicyInPW")) {
			...
			// Booking Underlying Quote (IL WC) in PW
			if (transactionsList.contains("BookPolicyInPW")) {
			...
			// Policy Issuance of Underlying Quote (IL BP)
			...
			// Policy Issuance of Underlying Quote (IL CA)
			if (transactionsList.contains("PolicyIssuanceOnAP")) {
				strRegressionIDUnderlined = "CUCA_09";
			...
			// Policy Issuance of Underlying Quote (IL WC)
			if (transactionsList.contains("PolicyIssuanceOnAP")) {
				strRegressionIDUnderlined = "CUWC_09";
			...
			// Policy Issuance of CU Quote (IL CU) 
			if (transactionsList.contains("PolicyIssuanceOnAP")) {
			...
			e.printStackTrace();
			se.verify().verifyEquals("CU10_IL_CURegression test  failed", true, false, true, test);
