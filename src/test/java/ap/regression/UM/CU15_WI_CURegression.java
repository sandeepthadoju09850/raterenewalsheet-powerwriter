...
public class CU15_WI_CURegression extends BaseTest{
	...
	public static void CU15_WI_CURegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,String strRegressionName, File file,XSSFWorkbook workbook, ExtentTest test) throws IOException 
	{
		...
		try {
			...
			String strRegressionIDUnderlined = "CUBP_17";
			...
			// UNDERLYING POLICY 1 CUBP_17 NEW QUOTE TRANSACTION
				...
				BT.ClickContinue(test); //Premium Modification
			 ...
			 // UNDERLYING POLICY 1 CUBP_17 REDO TRANSACTION
			if (transactionsList.contains("RedoNewQuote")) {
				...
				BT.ClickContinue(test);
				CP_InlandMarineCovSelection.CP_InlandMarineCovSelection_Details(strRegressionIDUnderlined, transaction, suspendSheet, test);
				BT.ClickContinue(test); //Accounts Receivable
				BT.ClickContinue(test); //Contractors Equipment
				// commented this screen as part of defect AP-48843 CP_ScheduledProperty.CP_ScheduledProperty_Details(strRegressionIDUnderlined, transaction, suspendSheet, test);
				BT.ClickContinue(test); //Signs Schedule
				BT.ClickContinue(test); //Valuable Papers
				BT.ClickContinue(test); //Loss History
				...
				BT.ClickContinue(test); //Underwriting Questions
				BT.ClickContinue(test); //Property Additional Interests
				//BT.ClickContinue(test); //Prior Carrier Information
				BT.ClickContinue(test); //Billing Information
				CP_Summary.CP_Summary_Details(strRegressionIDUnderlined,strRegressionName, transaction, suspendSheet, strAgentLinkBP, "Package", strRelease_SelectReleaseBP, strRole_SelectRoleAsBP,quote, test, file,  workbook);
				LoginPage.APLogoutDetails( strAgentLink,test );
			}

			// UNDERLYING POLICY 2 CUCA_14 NEW QUOTE TRANSACTION
			strRegressionIDUnderlined = "CUCA_14";
			if (transactionsList.contains("NewQuote")) {
			...
			// UNDERLYING POLICY 3 CUWC_14 NEW QUOTE TRANSACTION
			strRegressionIDUnderlined = "CUWC_14";
			if (transactionsList.contains("NewQuote")) {
				transaction = "NewQuote";String suspendSheet = ExcelOperations.getPageToBeSuspended(strRegressionID,transaction);
			...
			// COMMERCIAL UMBRELLA CU_15 NEW QUOTE TRANSACTION
			if (transactionsList.contains("NewQuote")) {
				transaction = "NewQuote";
				String suspendSheet = ExcelOperations.getPageToBeSuspended(strRegressionID,transaction);
				//LoginPage.APAppLogin(strRegressionIDUnderlined,   transaction,"No", test);
			...
			// Booking Underlying Quote (WI BP) in PW
			transaction = "BookPolicyInPW";
			String suspendSheet = ExcelOperations.getPageToBeSuspended(strRegressionID,transaction);
			
			if (transactionsList.contains("BookPolicyInPW")) {
				strRegressionIDUnderlined = "CUBP_17";
				LoginPage.APAppLogin(strRegressionIDUnderlined,   transaction,"No", test);
			...
			// Booking Underlying Quote (WI WC) in PW
			if (transactionsList.contains("BookPolicyInPW")) {
			...
			// Policy Issuance of Underlying Quote (WI BP)
			...
			e.printStackTrace();
			se.verify().verifyEquals("CU15_WI_CURegression test  failed", true, false, true, test);
