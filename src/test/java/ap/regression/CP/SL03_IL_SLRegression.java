...
import ap.pages.CP.CP_BoatAndMotorSchedule;
...
public class SL03_IL_SLRegression extends BaseTest{
	...
	public static void SL03_IL_SLRegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,String strRegressionName, File file,XSSFWorkbook workbook, ExtentTest test) throws IOException 
	{
		...
		CP_BoatAndMotorSchedule CP_BoatAndMotorSchedule = TestPageFactory.initElements(se, CP_BoatAndMotorSchedule.class);
		...
		try {
			...
			while (iteration < table.size()) {
				...
				if (transactionsList.contains("RedoNewQuote")) {
					...
					CP_Liablitycoverages.CP_Liablitycoverages_Details(strRegressionID, transaction, suspendSheet, test);
					BT.NavigateToTabs("Liability Options",test);
					CP_LiabilityOptions.CP_LiabilityOptions_Details(strRegressionID, transaction, suspendSheet, test);
					BT.ClickContinue(test);
					BT.ClickContinue(test);
					BT.ClickContinue(test);
					//BT.NavigateToTabs("Inland Marine Coverage Selection",test);
					CP_InlandMarineCovSelection.CP_InlandMarineCovSelection_Details(strRegressionID, transaction, suspendSheet, test);
					CP_ContractorsEquipmentScheduled.CP_ContractorsEquipmentScheduled_Details(strRegressionID, transaction, suspendSheet, test);
					CP_BoatAndMotorSchedule.CP_BoatAndMotorSchedule_Details(strRegressionID, transaction, suspendSheet, test); 
					CP_PremiumIndication.CP_PremiumIndication_Details(strRegressionID, transaction, suspendSheet, test);
					CP_LossHistoryExpRating.CP_LossHistoryExpRating_Details(strRegressionID, transaction, suspendSheet, test);
					
					CP_PremiumModification.CP_PremiumModification_Details(strRegressionID, transaction, suspendSheet, strAgentLink,strLOB,strRole_SelectRoleAs,strRelease_SelectRelease,quote, test);
				...
				if (transactionsList.contains("BookPolicyInPW")) {
					String transaction = "BookPolicyInPW";
					String suspendSheet = ExcelOperations.getPageToBeSuspended(strRegressionID,transaction);
				...
				if (transactionsList.contains("PolicyIssuanceOnAP")) {
					String transaction = "PolicyIssuanceOnAP";
					String suspendSheet = ExcelOperations.getPageToBeSuspended(strRegressionID,transaction);
			...
			e.printStackTrace();
			se.verify().verifyEquals("SL03_IL_SLRegression test  failed", true, false, true, test);
