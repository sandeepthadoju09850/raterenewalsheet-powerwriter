...
public class CU_GLPolicyInfo extends OR_CU {
	
	public void generalLiabPolicyInfo_Main(String strRegressionID, String transaction, String suspendSheet, ExtentTest test) throws IOException {
		try {
			
			if (suspendSheet.equalsIgnoreCase("GeneralLiabPolicyInfo")) {
				test.log(LogStatus.INFO, "Page Suspended :- ", "Transaction Step : " + transaction + "<br>" + "Page : " + suspendSheet);
				testTearDown(se, test);
			}
			List<Map<String, String>> table = ExcelOperations.getPagesData("GeneralLiabPolicyInfo", strRegressionID,transaction);
			int iteration = 0;
			while (iteration < table.size()) {
				...
				try {
					CU_GLCoverage CUGLCoveragePage = TestPageFactory.initElements(se, CU_GLCoverage.class);
					se.log().logTestStep("Entering details on General Liability Policy Info Page");
					test.log(LogStatus.INFO, "Entering details on General Liability Policy Info Page screen", "Transaction Step : " + transaction + "<br>" + "Page : General Liability Policy Info Page");
					se.verify().verifyEquals("General Liability Policy Info Page displayed", getPagecontainingtext("General Liability Policy Information").isDisplayed(),true, true, test);
					if(!strDetails_BTN.equals(constants.NA)){
						se.element().clickElement(getCU_GLPolicyInfo_Details_BTN(strDetails_BTN),test);
						se.verify().verifyEquals("General Liability Policy Info details is displayed ", getcommon_BTN_SaveAndNew().isDisplayed(),true, true,test);
					}
					if(!strAdd_BTN.equals(constants.NA)){
						se.element().clickElement(getCU_GLPolicyInfo_Add_BTN(strAdd_BTN),test);
					}
					if(!strRemove_BTN.equals(constants.NA)){
						se.element().clickElement(getCU_GLPolicyInfo_Remove_BTN(strRemove_BTN),test);
					}
					Thread.sleep(2000);
					CU_GLPolicyInfo(row,test);
					CUGLCoveragePage.generalLiabPolicyCoverage(strRegressionID, transaction, suspendSheet, test);
					...
					se.verify().verifyEquals("Failed to fill in details for CU_GL Policy Information screen Main Page ", true, false, true, test);
				}
				iteration++;
			}
		} catch (Exception e) {
	...
	public void CU_GLPolicyInfo(Map<String, String> row, ExtentTest test) throws IOException {
		
		String strUnderlyingQuotePolicyNum_TXT = (String) row.get("UnderlyingQuotePolicyNum_TXT");
		String strCompanyName_TXT = (String) row.get("CompanyName_TXT");
		String strCompanyNameFilterNeeded_TXT = (String) row.get("CompanyNameFilterNeeded_TXT");
		String strCompanyNameGroupNameOption_TXT = (String) row.get("CompanyNameGroupNameOption_TXT");
		String strEachOccurenceLimit_TXT = (String) row.get("EachOccurenceLimit_TXT");
		String strEachOccurenceLimitFilterNeeded_TXT = (String) row.get("EachOccurenceLimitFilterNeeded_TXT");
		String strGeneralAggregateLimit_TXT = (String) row.get("GeneralAggregateLimit_TXT");
		String strProductsCompletedOperationsAgg_CHK = (String) row.get("ProductsCompletedOperationsAgg_CHK");
		String strProductsCompletedOperationsAggLimit_TXT = (String) row.get("ProductsCompletedOperationsAggLimit_TXT");
		String strPersonalAndAdvertisingInjury_CHK = (String) row.get("PersonalAndAdvertisingInjury_CHK");
		String strPersonalAndAdvertisingInjuryLimit_TXT = (String) row.get("PersonalAndAdvertisingInjuryLimit_TXT");
		String strDescribeOperationsCoveredByThisPolicy_TXT = (String) row.get("DescribeOperationsCoveredByThisPolicy_TXT");
		String strPremiseOperationManualPremium_TXT = (String) row.get("PremiseOperationManualPremium_TXT");
		String strPremisesTable_TXT = (String) row.get("PremisesTable_TXT");
		String strPremisesTableFilterNeeded_TXT = (String) row.get("PremisesTableFilterNeeded_TXT");
		String strProductsCompletedOperationsManualPremium_TXT = (String) row.get("ProductsCompletedOperationsManualPremium_TXT");
		String strProductsTable_TXT = (String) row.get("ProductsTable_TXT");
		String strProductsTableFilterNeeded_TXT = (String) row.get("ProductsTableFilterNeeded_TXT");

		try {
			se.element().enterOrValidateText(getCU_GLPolicyInfo_UnderlyingQuotePolicyNo_TXT(strUnderlyingQuotePolicyNum_TXT),strUnderlyingQuotePolicyNum_TXT, test);
			se.element().selectPopupWithMagnifier(getCU_GLPolicyInfo_CompanyName_TXT(strCompanyName_TXT),
					getCU_GLPolicyInfo_CompanyName_Search_BTN(strCompanyName_TXT), strCompanyName_TXT, strCompanyNameFilterNeeded_TXT,
					strCompanyName_TXT, strCompanyNameGroupNameOption_TXT, constants.NA, constants.NA, test);
			se.element().selectPopupWithMagnifier(getCU_GLPolicyInfo_EachOccuranceLimit_TXT(strEachOccurenceLimit_TXT),
					getCU_GLPolicyInfo_EachOccuranceLimit_Search_BTN(strEachOccurenceLimit_TXT), strEachOccurenceLimit_TXT,
					strEachOccurenceLimitFilterNeeded_TXT, strEachOccurenceLimit_TXT, constants.NA, constants.NA,
					constants.NA, test);
			se.element().enterOrValidateText(getCU_GLPolicyInfo_GenearlAggLimit_TXT(strGeneralAggregateLimit_TXT), strGeneralAggregateLimit_TXT,
					test);
			se.element().checkUncheckOrValidate(getCU_GLPolicyInfo_ProductCompOperAgg_CHK(strProductsCompletedOperationsAgg_CHK),
					strProductsCompletedOperationsAgg_CHK, test);
			se.element().enterOrValidateText(getCU_GLPolicyInfo_ProductCompOperAggLimit_TXT(strProductsCompletedOperationsAggLimit_TXT),
						strProductsCompletedOperationsAggLimit_TXT, test);
			
			se.element().checkUncheckOrValidate(getCU_GLPolicyInfo_PersonalAndAdvInjury_CHK(strPersonalAndAdvertisingInjury_CHK),
					strPersonalAndAdvertisingInjury_CHK, test);

				se.element().enterOrValidateText(getCU_GLPolicyInfo_PersonalAndAdvInjuryLimit_TXT(strPersonalAndAdvertisingInjuryLimit_TXT),
						strPersonalAndAdvertisingInjuryLimit_TXT, test);

			se.element().enterOrValidateText(getCU_GLPolicyInfo_DescOperations_TXTArea(strDescribeOperationsCoveredByThisPolicy_TXT),
					strDescribeOperationsCoveredByThisPolicy_TXT, test);

			se.element().enterOrValidateText(getCU_GLPolicyInfo_ManualGLPrem_PremiseManualPrem_TXT(strPremiseOperationManualPremium_TXT),
					strPremiseOperationManualPremium_TXT, test);
			se.element().selectPopupWithMagnifier(getCU_GLPolicyInfo_ManualGLPrem_PremisesTbl_TXT(strPremisesTable_TXT),
					getCU_GLPolicyInfo_ManualGLPrem_PremisesTbl_Search_BTN(strPremisesTable_TXT), strPremisesTable_TXT,
					strPremisesTableFilterNeeded_TXT, strPremisesTable_TXT, constants.NA, constants.NA, constants.NA,
					test);
			se.element().enterOrValidateText(getCU_GLPolicyInfo_ManualGLPrem_ProductsManualPrem_TXT(strProductsCompletedOperationsManualPremium_TXT),
					strProductsCompletedOperationsManualPremium_TXT, test);
			se.element().selectPopupWithMagnifier(getCU_GLPolicyInfo_ManualGLPrem_PriductsTbl_TXT(strProductsTable_TXT),
					getCU_GLPolicyInfo_ManualGLPrem_ProductsTabl_Search_BTN(strProductsTable_TXT), strProductsTable_TXT,
					strProductsTableFilterNeeded_TXT, strProductsTable_TXT, constants.NA, constants.NA, constants.NA,
					test);
			se.element().Click(getNext(), test);
			se.element().waitForElement(Wariningyesbutton, 10);
			...
			se.verify().verifyEquals("Failed to fill in details for CU_geenral Liability Policy Information Page ", true, false, true, test);
		}
	}
	
	public void CU_GeneralLiabCovInfo(Map<String, String> row, ExtentTest test) throws IOException {
		...
		try {
			...
			se.verify().verifyEquals("Failed to fill in details for CU_General liability Coverage screen ", true, false, true, test);
		}
	}
}
