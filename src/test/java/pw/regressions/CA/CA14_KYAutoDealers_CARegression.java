...
import pw.pages.CA.CA_BusinessInterruption;
...
public class CA14_KYAutoDealers_CARegression extends BaseTest{
	ExtentReports extent;
	ExtentTest test;
		
	public static void KYAutoDealers_CARegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,File file,XSSFWorkbook workbook, ExtentTest test) throws IOException {
		...
		CA_BusinessInterruption CA_BusinessInterruptionPage = TestPageFactory.initElements(se, CA_BusinessInterruption.class);
		...
		try {
			...
			if (transactionsList.contains("NewQuote")) {
				...
				CA_BusinessInterruptionPage.BusinessInterruption(strRegressionID, transaction, suspendSheet, test);
			...
			if(transactionsList.contains("FlatEndorsement1")){
				...
				CA_DriveAwayCollisionPage.DriveAwayCollision(strRegressionID, transaction, suspendSheet, test);
				CommonMethods.NavigateTo("Garage Keepers (3)", test);
				CA_GarageKeeperesPage.CAGarageKeepersPage(strRegressionID, transaction, suspendSheet, test);
			...
			e.printStackTrace();
			se.verify().verifyEquals("KYAutoDealersCARegression test  failed", true, false);
			testTearDown(se);
		}
	}
