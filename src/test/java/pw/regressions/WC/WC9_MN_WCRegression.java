...
public class WC9_MN_WCRegression extends BaseTest {
	ExtentReports extent;
	ExtentTest test;
	
	public static void MN_WCRegressionMethod(Browsers myBrowser, SeHelper se, Map<String, Object> params,String strRegressionID,File file,XSSFWorkbook workbook, ExtentTest test) throws IOException {
		...
		try {
			...
			if(transactionsList.contains("ReviseQuote")){
				...
				CommonMethods.NavigateTo("Minnesota",test);
			...
			e.printStackTrace();
			se.verify().verifyEquals("MN WC Regression test failed", true, false);
			testTearDown(se);
		}
	}
}
